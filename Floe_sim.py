...
MAX_DIST = 200
MAX_LOST_FRAMES = 30 # number of frames I want to predict a lost Floe's path
...
class Floe_Manager:
    def __init__(self):
        ...
        self.next_id = 0
        self.grid = gr.Grid(10, 0, 100, 0, 300) # a grid matching the simulator area
        ...
        for floe in self.active_floes.values():
            if floe.predicted_state is not None:
                ...
                if floe.measurement is None:
                    ...
                    px, py = floe.state[0, 0], floe.state[1, 0] # pos in x and y dir
                    vx, vy = floe.state[2, 0], floe.state[3, 0] # velocity in x and y direction
                    self.grid.add_velocity_vector((px, py), (vx, vy))
    ...
    def display_rasters(self):
        for floe in self.active_floes.values():
            self.display_raster(floe.raster)

    def print_floe_paths(self):
        for floe in self.active_floes.values():
            print(floe.path)
    ...
    def optical_flow_2(self, prev_image, curr_image):
        ...
        if prev_keypoints is None:
            fplt.visualize_optical_flow(prev_image, curr_image, avg_du=0, avg_dv=0,
                                        good_prev_keypoints=[],
                                        good_curr_keypoints=[])
            raise ValueError("No prev keypoints found")

        lk_params = dict(
            winSize=(21, 21),           # Larger window for large motion
            maxLevel=25,                 # More pyramid levels
        ...
        if curr_keypoints is None or status is None:
            raise ValueError("no curr keypoints found or status is false")
        ...
        if len(good_prev) == 0:
            fplt.visualize_optical_flow(prev_image, curr_image, avg_du=0, avg_dv=0,
                                        good_prev_keypoints=[],
                                        good_curr_keypoints=[])
            raise ValueError("No good prev keypoints found")
    ...
    def get_cost_matrix(self, prev_floes, curr_floes):
        ...
        DIST = 30 # distance [meter]
        MAX_IOU = 1.1  # the lower the better. from 0 to 1

        for i, prev_floe in enumerate(prev_floes):
            y_pred = prev_floe.predicted_state[:2].flatten()  # Predicted position [x y]
            S = np.dot(np.dot(prev_floe.kalmanfilter.C, prev_floe.kalmanfilter.P), prev_floe.kalmanfilter.C.T) + prev_floe.kalmanfilter.R # uncertainty in prev_floe
    ...
    def update_matched_floes(self, prev_match, curr_match, detections, floe_ids, cost_matrix):
        ...
        for prev_idx, curr_idx in zip(prev_match, curr_match):
            ...
            try:
                ...
                # self.display_image(curr_image)

                # Doing morphological operations to remove noise
                ...
                #  Optical Flow
                avg_du, avg_dv, good_prev, good_curr = self.optical_flow_2(prev_image, curr_image)
    ...
    def backtrack_floe(self, floe_ID, added_at_time, backtrack_steps=10):
        ...
        # Phase 1: Use stored measurements in reverse
        ...
        # Phase 2: Use velocity field for further backtracking
        for _ in range(backtrack_steps):
            reverse_kf.predict(A_inv)

            x, y = reverse_kf.x[0, 0], reverse_kf.x[1, 0]
            i, j = self.grid.get_grid_index(x, y)
            vel, dist = self.grid.get_velocity_and_distance(i, j)
            print("dist:", dist)
            print("R value:", 0.3 * (1 + 0.01 * dist))
...
class Raster:
    ...
    def print(self):
        ...
        print("North_min:", self.north_max)
        print("East_min:", self.east_min)
